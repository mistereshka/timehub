...
/** The cards inside an album, most played first. */
export const sortTracks = (tracks: LibraryItem[]): LibraryItem[] =>
  [...tracks].sort((a, b) => b.progress - a.progress || a.title.localeCompare(b.title))
        ...
        {tracks.length > 0 && (
          <Field label={t('lib.albumTracks')}>
            <div className="album-tracks">
              {sortTracks(tracks).map((tr, n) => (
                <button key={tr.id} type="button" className="album-track" onClick={() => onOpenItem?.(tr)}>
                  <span className="album-track-num">{n + 1}</span>
                  <span className="grow truncate">{tr.title}</span>
                  <span className="muted nowrap">
                    {libraryStatusLabel(tr.status, tr.kind, t)} · {t('lib.progressOpen', { n: tr.progress, unit: t('lib.unit.music') })}
                  </span>
                </button>
              ))}
            </div>
